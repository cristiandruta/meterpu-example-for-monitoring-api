...
#ifdef METERPU_INTEGRATE_WITH_MF
#include <sys/time.h>
#include <string.h>
#endif
...
#ifdef METERPU_INTEGRATE_WITH_MF
static void
meterpu_translate_time_to_string_timespec(const struct timespec *ts, char* timestamp, const char* format, int in_milliseconds)
{
    char fmt[64];
    char buf[64];
    struct timeval tv;
...
char*
meterpu_mf_api_get_time_from_timespec(const struct timespec *ts)
{
    char* timestamp = (char *)malloc(sizeof(char) * 64);
    meterpu_translate_time_to_string_timespec(ts, timestamp, "%Y-%m-%dT%H:%M:%S.%%6u", 1);
    return timestamp;
}
#endif
...
#ifndef METERPU_INTEGRATE_WITH_MF
